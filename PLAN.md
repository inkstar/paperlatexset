...
### [UTC+8 2026-02-25 13:00] Phase 4.34 提取页三列同高布局修正（编辑区/LaTeX 区与识别区一致）
- 目标
  - 修复“中间题目编辑区与右侧 LaTeX 区高度偏短”的视觉问题。
  - 让桌面端三列（左识别、中编辑、右代码）保持同一工作区高度。
- 改动文件
  - `App.tsx`
  - `PLAN.md`
- 验收标准
  - `识别与编辑` 页面在 `lg` 尺寸下三列等高。
  - 中间编辑表格和右侧 LaTeX 面板可在同高容器内滚动，不再明显短于左列。
  - 移动端布局不回归（仍为纵向堆叠）。
- 风险与回滚
  - 风险：不同浏览器下 `vh` 高度计算存在轻微差异。
  - 回滚：保留主容器高度控制，改回中右独立高度策略。
- 发布状态
...
- 目标
  ...
  - 降低“本机后端启动异常”对主流程可用性的影响。
- 改动文件
  - `services/geminiService.ts`
  - `README.md`
  - `PLAN.md`
- 验收标准
  - `/api/analyze` 或 `/api/parse-latex` 返回 `BACKEND_UNREACHABLE` 时，自动回退前端 Gemini 识别。
  - 若未配置 `VITE_GEMINI_API_KEY`，给出明确提示，不再静默失败。
  - 识别结果继续走既有 LaTeX 规范清洗（`normalizeLatexContent`）。
- 风险与回滚
  - 风险：前端直连模型无法复用后端统一日志与权限控制。
  - 回滚：仅保留后端路径，移除前端兜底分支。
- 发布状态
  - 已提交并推送（commit hash 见本阶段提交）。

### [UTC+8 2026-02-24 15:13] Phase 4.31 组卷筛选交互增强（下拉筛选 + 标签清除 + 来源/年份快捷筛选）
- 目标
  - 提升组卷页筛选效率，减少手输与误筛选成本。
  - 让知识点/题型/来源/年份筛选状态可见且可单独清除。
- 改动文件
...
- 验收标准
  - 四个筛选项统一为下拉选择（全部/指定值）。
  - 已启用筛选以标签形式展示，支持单击移除单项筛选。
  - 快捷筛选扩展到来源与年份，和知识点/题型行为一致。
- 风险与回滚
  - 风险：极端长选项在下拉中可读性下降。
  - 回滚：保留快捷筛选按钮，恢复自由输入模式。
- 发布状态
  - 已提交并推送（commit hash 见本阶段提交）。
...
- 目标
  ...
  - 让导出参数在页面刷新后保持，减少重复录入成本。
- 改动文件
  - `components/ComposerPage.tsx`
  - `README.md`
  - `PLAN.md`
- 验收标准
  - 导出 LaTeX 前校验 `choiceGap/solutionGap` 单位格式与 `lineSpacing` 范围（0.8~3）。
  - 参数非法时前端直接提示并阻断导出请求。
  - 导出参数写入 `localStorage`，刷新后可恢复上次设置。
- 风险与回滚
  - 风险：校验过严可能拦截部分历史可用输入格式。
  - 回滚：保留持久化，仅放宽校验规则为弱校验。
- 发布状态
  - 已提交并推送（commit hash 见本阶段提交）。

### [UTC+8 2026-02-24 12:18] Phase 4.28 导出参数易用性增强（默认恢复与输入提示）
- 目标
  - 降低导出参数误填成本，提供一键恢复默认与单位/范围提示。
- 改动文件
  - `components/ComposerPage.tsx`
  - `PLAN.md`
- 验收标准
  - 桌面/移动试卷篮的 LaTeX 导出设置均提供“恢复默认”按钮。
  - 标题占位提示明确“默认当日日期”。
  - 设置区显示留白单位与行距建议文案。
- 风险与回滚
  - 风险：设置区信息增加可能轻微提升界面密度。
  - 回滚：保留功能，仅隐藏提示文案。
- 发布状态
  - 已提交并推送（commit hash 见本阶段提交）。
...
- 改动文件
  - `PLAN.md`
- 验收标准
  - Phase 4.24 发布状态为 `de68f75`。
  - Phase 4.25 发布状态为 `020bf56`。
- 风险与回滚
  - 风险：回滚后 hash 与线上版本短暂不一致。
  - 回滚：追加回滚注记，不改历史 hash。
- 发布状态
  - 已提交并推送（commit hash 见本阶段提交）。
